...
**大规模压测（2026-08-24）**：316 份文档（txt/docx/xlsx/pdf/扫描件）× 300 条用例 × 5 种角色：

| 类别 | 结果 |
|---|---|
| 正向问答（大库精准命中、跨格式、聚合、多轮） | **100/100** |
| 幻觉陷阱 | **100/100** 全部正确拒答 |
| 越权探测（跨部门 + 密级不足） | **100/100 零泄露** |
| **总计** | **300/300 = 100%** |

压测逼出并修复一个真实缺陷：批量入库时的解析任务孤儿态（已加启动恢复机制）。
